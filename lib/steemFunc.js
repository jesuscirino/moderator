...
export const getJsonMetadata = async (author, permlink) =>{
    const query = {
        start_author: author,
    ...
    const arrayBlogs = await client.database
                                   .getDiscussions('blog', query)
    const jsonMetadata = arrayBlogs[0].json_metadata
    if (!jsonMetadata)
        throw new Error('no JsonMeta')
